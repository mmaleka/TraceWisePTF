...
function getCofcIdFromURL() {
  ...
  console.log("sfsdfdsfds: ", id.replace(/^0+/, '') || '0')
...
async function loadPalletComponents() {
  ...
  console.log("load pallet components: ", cofcId);
  
  if (!cofcId) return;

  try {
    const response = await fetch(`http://127.0.0.1:8000/api/certificate/components/?certificate=${cofcId}`, {
      headers: {
        "Authorization": `Bearer ${localStorage.getItem("authToken")}`,
        "Content-Type": "application/json"
      }
...
async function moveToPallet(row) {
  ...
  // let cofcId = document.getElementById("cocNumber").dataset.id;  // Example: <span id="cocNumber" data-id="1">0001</span>
  if (!cofcId) {
...
async function verifyCofC() {
  ...
  try {
    // const response = await fetch(`https://tracewiseptf.onrender.com/api/certificate/verify/?certificate=${cofcId}`, {
    const response = await fetch(`http://127.0.0.1:8000/api/certificate/verify/?certificate=${cofcId}`, {
      headers: {
        "Authorization": `Bearer ${token}`,
        "Content-Type": "application/json",
      },
...
async function printCofC() {
  ...
  try {
    // Fetch CofC details
    const cocRes = await fetch(`http://127.0.0.1:8000/api/certificate/cofc/${cofcId}/`, {
      headers: {
        "Authorization": `Bearer ${token}`,
        "Content-Type": "application/json",
      },
    });
    if (!cocRes.ok) throw new Error("Failed to fetch CofC details");
    const cocData = await cocRes.json();

    // Fetch CofC component list (pallet)
    const componentsRes = await fetch(`http://127.0.0.1:8000/api/certificate/components/?certificate=${cofcId}`, {
      headers: {
        "Authorization": `Bearer ${token}`,
        "Content-Type": "application/json",
      },
    });
    if (!componentsRes.ok) throw new Error("Failed to fetch CofC components");
    const components = await componentsRes.json();

    if (components.length === 0) {
      alert("❌ Cannot generate CofC PDF. Empty Pallet.");
      return;
    }

    // Convert component list to palletData format
    const palletData = components.map(item => ({
      shell: item.serial,
      cast: item.cast_code,
      heat: item.heat_code
    }));

    // Now call your PDF generator
    generateCofCPDF(
      cocData.coc_number,
      cocData.product_name || "Unknown Product",
      cocData.user,
      new Date(cocData.date).toLocaleDateString(),
      palletData
    );
  } catch (err) {
    console.error("❌ Error printing CofC:", err);
    alert("❌ Error generating CofC PDF. See console.");
  }
}
...
function generateCofCPDF(cocNumber, product, user, date, palletItems = []) {
  ...
  doc.addImage(logoBase64, 'PNG', 20, 5, 60, 30);

  doc.setFontSize(14);
  doc.text("CERTIFICATE OF CONFORMANCE", 130, 25, null, null, "center");
  ...
  // Save PDF
  doc.save(`CertificateOfConformance_${cocNumber}.pdf`);
}
